...
//Criado Por : Vinicius Manoel em Novembro/2024
//Trabalho para faculdade UNIP PIM-IV
//Nome do perfil no github : DevViniNine
//Github : https://github.com/DevViniNine
...
gboolean on_delete_event(GtkWidget *widget, GdkEvent *event, gpointer data) {
    gtk_main_quit();  // Finaliza o loop principal do GTK e fecha o aplicativo
    return FALSE;     // Não propaga o evento para os outros manipuladores
}
...
//------------------------------------BOTA PARA ABRIR A TELA DE ATUALIZAÇÃO DE DADOS-----------------------
...
//------------------------------------INICIO DA PARTE DE ATUALIZAÇÃO DE DADOS-------------------------------
...
//------------------------------------BOTAO PARA ABRIR A TELA DE ATUALIZAÇÃO DE DADOS-----------------------
...
void on_menu_atualizar_dados_clicked(GtkButton *button, gpointer data) {
    ...
    // Lê as linhas restantes e preenche o ListStore
    while (fgets(line, sizeof(line), file)) {
        char *tokens[15] = {0};
        int i = 0;
        ...
        // Tratamento dos valores de Resíduo e Custo essa parte meu deu muita dor de cabeça, pois sem tratar os dados os valores aparecia zerados, e eu achava que poderia resovler de outra forma.
        double residuo = (tokens[13] != NULL && strlen(tokens[13]) > 0) ? atof(tokens[13]) : 0.0;
        double custo = (tokens[14] != NULL && strlen(tokens[14]) > 0) ? atof(tokens[14]) : 0.0;


        GtkTreeIter iter;
        gtk_list_store_append(liststore, &iter); // Gera um novo iterador
        gtk_list_store_set(liststore, &iter,
                           0, tokens[0],   // Nome da Empresa
                           1, tokens[1],   // Razão Social
                           2, tokens[2],   // Nome Fantasia
                           3, tokens[3],   // Responsável
                           4, tokens[4],   // CPF
                           5, tokens[5],   // CNPJ
                           6, tokens[6],   // Telefone
                           7, tokens[7],   // Email
                           ...
                           10, tokens[10], // Estado
                           11, tokens[11], // CEP
                           12, tokens[12], // Data de Abertura
                           13, residuo,    // Resíduo (convertido para double)
                           14, custo,      // Custo (convertido para double)
                           -1);
    }
...
void on_confirma_atualizar_dados_clicked(GtkButton *button, gpointer data) {
    ...
    gchar *novo_residuo_formatado = g_strdup(novo_residuo);
    gchar *novo_custo_formatado = g_strdup(novo_custo);

    for (int i = 0; novo_residuo_formatado[i] != '\0'; i++) {
        if (novo_residuo_formatado[i] == ',') {
            novo_residuo_formatado[i] = '.';
        }
    }

    for (int i = 0; novo_custo_formatado[i] != '\0'; i++) {
        if (novo_custo_formatado[i] == ',') {
            novo_custo_formatado[i] = '.';
        }
    }

    // Valida se existe mais de um ponto decimal nas entradas de resíduos e custos
    int ponto_residuo = 0;
    int ponto_custo = 0;

    for (int i = 0; novo_residuo_formatado[i] != '\0'; i++) {
        if (novo_residuo_formatado[i] == '.') ponto_residuo++;
    }

    for (int i = 0; novo_custo_formatado[i] != '\0'; i++) {
        if (novo_custo_formatado[i] == '.') ponto_custo++;
    }

    // Verifica se há mais de um ponto decimal
    if (ponto_residuo > 1) {
        gtk_label_set_text(status_label, "Resíduo não pode ter mais de um ponto decimal.");
        g_free(novo_residuo_formatado);
        g_free(novo_custo_formatado);
        return;
    }

    if (ponto_custo > 1) {
        gtk_label_set_text(status_label, "Custo não pode ter mais de um ponto decimal.");
        g_free(novo_residuo_formatado);
        g_free(novo_custo_formatado);
        return;
    }
    ...
    gdouble novo_residuo_val = atof(novo_residuo_formatado);
    gdouble novo_custo_val = atof(novo_custo_formatado);
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
        ...
        while (fgets(line, sizeof(line), file)) {
            ...
            if (strcmp(tokens[0], nome_empresa) == 0 && strcmp(tokens[5], cnpj) == 0) {
                // Formata corretamente os valores de resíduos e custos com duas casas decimais
...
void on_atuaizar_arquivo_clicked(GtkButton *button, gpointer data) {
    GtkBuilder *builder = (GtkBuilder *)data;

    // Obtém os widgets necessários
    GtkTreeView *treeview = GTK_TREE_VIEW(gtk_builder_get_object(builder, "atualizacao_treeview"));
    GtkListStore *liststore = GTK_LIST_STORE(gtk_builder_get_object(builder, "liststore2"));
    GtkLabel *status_label = GTK_LABEL(gtk_builder_get_object(builder, "status_atualiza_dados"));

    // Limpa o ListStore antes de recarregar
    gtk_list_store_clear(liststore);

    // Abre o arquivo CSV para leitura
    FILE *file = fopen(INDUSTRIAS_CSV, "r");
    if (!file) {
        gtk_label_set_text(status_label, "Erro ao abrir o arquivo de dados.");
        return;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (i < 15 || tokens[0] == NULL) {
            continue;
        }

        // Remove o caractere de nova linha das colunas
        for (int j = 0; j < i; j++) {
            if (tokens[j]) {
                char *newline_pos = strchr(tokens[j], '\n');
                if (newline_pos) *newline_pos = '\0';
        ...
        // Tratamento dos valores de Resíduo e Custo
        double residuo = (tokens[13] != NULL && strlen(tokens[13]) > 0) ? atof(tokens[13]) : 0.0;
        double custo = (tokens[14] != NULL && strlen(tokens[14]) > 0) ? atof(tokens[14]) : 0.0;

        // Adiciona os dados no ListStore
        GtkTreeIter iter;
        gtk_list_store_append(liststore, &iter);  // Gera um novo iterador
        gtk_list_store_set(liststore, &iter,
                           0, tokens[0],   // Nome da Empresa
    ...
    // Atualiza o status
    gtk_label_set_text(status_label, "Dados recarregados com sucesso.");
}


//------------------------------------FIM DA PARTE DE ATUALIZAÇÃO DE DADOS----------------------------------



//------------------------------------VINICIUS MANOEL - SOFTWARE CRIADO EM NOVEMBRO/2024--------------------
//Criado Por : Vinicius Manoel em Novembro/2024
//Trabalho para faculdade UNIP PIM-IV
//Nome do perfil no github : DevViniNine
//Github : https://github.com/DevViniNine
...
//------------------------------------FUNÇÃO PARA O BOTAO ABRIR O DIRETORIO DO RELATORIO---------------------
...
//------------------------------------BOTA SAIR DA TELA DE ATUALIZAÇÃO DE DADOS------------------------------

void on_atualizar_dados_voltar_clicked(GtkButton *button, gpointer user_data) {
    GtkBuilder *builder = (GtkBuilder *)user_data;

    // Obtém as referências das entradas e labels
    GtkEntry *entry_nome = GTK_ENTRY(gtk_builder_get_object(builder, "nome_da_industria_selecionada"));
    GtkEntry *entry_cnpj = GTK_ENTRY(gtk_builder_get_object(builder, "cnpj_da_industria_selecionada"));
    GtkLabel *label_residuo = GTK_LABEL(gtk_builder_get_object(builder, "atualiza_residuo_atual"));
    GtkLabel *label_custo = GTK_LABEL(gtk_builder_get_object(builder, "atualiza_custo_atual"));
    ...
    // Limpa os campos das entries e labels
    gtk_entry_set_text(entry_nome, "");
    gtk_entry_set_text(entry_cnpj, "");
    gtk_label_set_text(label_residuo, "");
    gtk_label_set_text(label_custo, "");

    // Troca para a visualização "view_menu_principal" na stack
...
//------------------------------------FUNÇÃO PARA ABRIR TELA DE CADASTRO DE INDUSTRIAS------------------------
...
int main(int argc, char *argv[]) {
    ...
    gtk_builder_add_callback_symbols(
        ...
        "on_atualizar_dados_voltar_clicked",G_CALLBACK(on_atualizar_dados_voltar_clicked),
        "on_atuaizar_arquivo_clicked", G_CALLBACK(on_atuaizar_arquivo_clicked),
        NULL
    );
